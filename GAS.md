# GAS.md — Google Apps Script
## Sistema de Seguimiento a Egresados — Universidad en el Campo

Este archivo contiene el código completo del Web App de Google Apps Script.
La fuente de verdad versionada es **`gas/Code.gs`** + `gas/appsscript.json`;
este `.md` es una copia legible para referencia. El script está **vinculado**
a la hoja `Seguimiento Egresados — Universidad en el Campo`.

- Script ID: `1pTLN-0XUpb8D7ERkRTkBv5TBIwDIJrrhWJQJG4MExkr5jF4V17i2fpRX` (`gas/.clasp.json`)
- Spreadsheet ID: `1vDxjtAe2bUPx2gUt1HMI2rlx6r1FfGBYKnv6BjaJd0k`
- Despliegue de producción (el de `VITE_GAS_URL`):
  `AKfycbw2EVV2WxhZ8bI4RxJSqLCLJyfzuaE_FIugP16Pbe4ewAQm6qHRh7M0Wd1VRWuBgCZGMA`

---

## Flujo de trabajo con clasp (recomendado)

`clasp` (`@google/clasp`, global) ya está autenticado como el dueño de la hoja.
Los scripts npm hacen `cd gas` y leen `gas/.clasp.json` (solo `scriptId`).

```bash
npm run gas:pull                       # traer el código remoto a gas/
npm run gas:push                       # subir gas/Code.gs + appsscript.json (a HEAD)
npm run gas:version -- "descripción"   # crear versión inmutable → imprime "Created version N"
npm run gas:redeploy -- -V N -d "..."  # apuntar el despliegue de PRODUCCIÓN a la versión N
npm run gas:deployments                # listar despliegues
npm run gas:logs                       # ver logs de ejecución
```
...
```bash
npm run gas:push
npm run gas:version -- "Mes 7: filtros cruzados"   # → Created version 5
npm run gas:redeploy -- -V 5 -d "Mes 7"
```

**Importante:** para no cambiar la URL del Web App, siempre se hace
`redeploy` sobre el despliegue existente (`AKfycbw2…`), nunca `clasp deploy`
(que crea uno nuevo con otra URL). El despliegue `@HEAD` (`AKfycbygk…`) sirve
siempre el último `push` y se usa solo para pruebas.

## Alternativa manual (sin clasp)

1. Abrir el Google Sheet → Extensiones → Apps Script.
2. Pegar el contenido de `gas/Code.gs`. Guardar con Ctrl+S.
3. **Implementar → Gestionar implementaciones →** editar la existente →
   "Versión nueva" → Implementar. (No crear una implementación nueva:
   cambiaría la URL.)
4. Config del Web App: ejecutar como **Yo**, acceso **Cualquier usuario**.
...
- **Cada modificación** al código requiere una nueva implementación
  en GAS para que los cambios sean efectivos. En particular, la acción
  `?action=instituciones` (usada por `/admin/instituciones` desde el Mes 6)
  solo responde tras volver a desplegar el Web App.
